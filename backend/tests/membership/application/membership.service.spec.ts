...
} from '../../../src/membership/domain/membership.repository';
...
describe('MembershipService', () => {
  ...
  beforeEach(async () => {
    ...
    const mod: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: PinoLogger,
          useValue: { warn: jest.fn(), error: jest.fn(), debug: jest.fn() },
        },
      ],
    }).compile();
  ...
  describe('createCheckoutSession', () => {
    it('rejects free plan', async () => {
      await expect(
        service.createCheckoutSession('free', 'u1'),
      ).rejects.toBeInstanceOf(BadRequestException);
    });

    it('rejects unknown plan', async () => {
      await expect(
        service.createCheckoutSession('unknown', 'u1'),
    ...
    it('rejects when already on active plan', async () => {
      ...
      await expect(
        service.createCheckoutSession('curator', 'u1'),
  ...
  describe('enforceBorrowLimit', () => {
    ...
    it('throws when at limit', async () => {
      ...
      await expect(service.enforceBorrowLimit('u1')).rejects.toThrow(
        /borrow limit/,
      );
    });
  });
